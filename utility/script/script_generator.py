...
def generate_script(topic):
    ...
    Fallback chain: openrouter -> gemini -> groq -> openai -> local (offline)
    """
    config = get_config()
    user_provider = config.get_llm_provider()
    
    if user_provider == 'auto':
        # Local is the ultimate fallback when all cloud APIs fail
            ...
            for model in models_to_try:
                try:
                    ...
                    if provider == 'gemini':
                        ...
                        # Use local LLM (Qwen) for offline script generation
